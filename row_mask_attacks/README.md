...
- `--slurm_run X` (without `--job_num`) to generate `run_X.slurm` for just that experiment.
- `--slurm_run X --job_num Y` to run a single job from that experiment.
- `--job_num Y` (without `--slurm_run`) to run a single job from the full experiment set.
...
When run with `--slurm_run X --job_num Y`, it runs the attack associated with the index `Y` for the experiment with `slurm_run = X`.
...
Contains the configurations of experimental parameters. Used both by `run_row_mask_attack.py` to run experiments (only those with `dont_run = False` and where `slurm_run` matches the corresponding command line parameter, see below) and `analyze.py` to analyze experiments.

## Workflow

The general workflow for running experiments is then as follows:

1. Add a set of experimental parameters to `experiments.py`, including a unique `slurm_run` value for each entry. We recommend configuring 5 distinct seeds.
...
3. Run `run_row_mask_attack.py --slurm_run X` to generate `run_X.slurm` for the desired experiment.
4. Do `sbatch run_X.slurm` on the SLURM cluster.
